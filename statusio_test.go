...
func Test_StatusioApi_Maintenance_Schedule(t *testing.T) {
	result, err := api.MaintenanceSchedule(Maintenance{
		...
		MessageSubject:            "Example Notification Message Subject",
		DatePlannedStart:          time.Now().Add(1 * time.Hour).Format("01/02/2006"),
		TimePlannedStart:          time.Now().Add(1 * time.Hour).Format("15:04"),
		DatePlannedEnd:            time.Now().Add(2 * time.Hour).Format("01/02/2006"),
